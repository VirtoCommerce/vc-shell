...
export class OrderModuleShipmentsClient extends AuthApiBase {
     ...
     * @return Success
     */
    searchOrderShipments(body?: ShipmentSearchCriteria | undefined): Promise<ShipmentSearchResult> {
        let url_ = this.baseUrl + "/api/order/shipments/search";
        url_ = url_.replace(/[?&]$/, "");
        ...
        return this.transformOptions(options_).then(transformedOptions_ => {
            ...
            return this.processSearchOrderShipments(_response);
        });
    }

    protected processSearchOrderShipments(response: Response): Promise<ShipmentSearchResult> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
...
export class ShipmentSearchCriteria implements IShipmentSearchCriteria {
    orderId?: string | undefined;
    orderNumber?: string | undefined;
    fulfillmentCenterId?: string | undefined;
    shipmentMethodCode?: string | undefined;
    ...
    constructor(data?: IShipmentSearchCriteria) {
        if (data) {
            for (var property in data) {
                if (data.hasOwnProperty(property))
                    (<any>this)[property] = (<any>data)[property];
    ...
    static fromJS(data: any): ShipmentSearchCriteria {
        data = typeof data === 'object' ? data : {};
        let result = new ShipmentSearchCriteria();
        result.init(data);
        return result;
    }
...
export interface IShipmentSearchCriteria {
    orderId?: string | undefined;
    orderNumber?: string | undefined;
    fulfillmentCenterId?: string | undefined;
    shipmentMethodCode?: string | undefined;
    ...
    init(_data?: any) {
        if (_data) {
            ...
            if (Array.isArray(_data["results"])) {
                this.results = [] as any;
                for (let item of _data["results"])
                    this.results!.push(OrderShipment.fromJS(item));
            }
        }
    }
