...
function requireContext(method: string): never {
  throw new Error(`[vc-shell] ${method} requires blade context.\n${CONTEXT_ERROR_SUFFIX}`);
}
...
export function useBlade<TOptions = Record<string, unknown>>(): UseBladeReturn<TOptions> {
  ...
  const _stack: IBladeStack | undefined =
    (getCurrentInstance() ? inject(BladeStackKey, undefined) : undefined) ?? bladeStackInstance;
  const _messaging =
    (getCurrentInstance() ? inject(BladeMessagingKey, undefined) : undefined) ?? bladeMessagingInstance;
