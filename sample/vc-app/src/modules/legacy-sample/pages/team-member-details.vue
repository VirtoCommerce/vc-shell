...
const title = computed(() =>
  props.param ? userDetails.value.firstName + " " + userDetails.value.lastName : t("TEAM.PAGES.DETAILS.TITLE")
);
...
const bladeToolbar = ref<IBladeToolbar[]>([
  {
    ...
    async clickHandler() {
      if (isValid.value) {
        ...
        showError(unref(computed(() => t("TEAM.PAGES.DETAILS.FORM.NOT_VALID"))));
      }
    },
  ...
  {
    id: "resend",
    title: computed(() => t("TEAM.PAGES.DETAILS.TOOLBAR.RESEND")),
    icon: "fas fa-paper-plane",
    async clickHandler() {
...
async function onBeforeClose() {
  if (modified.value) {
    return await showConfirmation(unref(computed(() => t("TEAM.ALERTS.CLOSE_CONFIRMATION"))));
  }
}
