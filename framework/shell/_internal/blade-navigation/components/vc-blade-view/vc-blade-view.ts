...
import { BladeInstanceKey, NavigationViewLocationKey, BladeBackButtonKey } from "@framework/injection-keys";
import { BladeVNode, CoreBladeExposed, IBladeInstance } from "@core/blade-navigation/types";
...
export const VcBladeView = defineComponent({
  ...
  setup(props: BladeViewProps, { attrs, slots }: SetupContext) {
    ...
    if (bl.value) {
      provide(NavigationViewLocationKey, bl.value);
    }

    provide(BladeBackButtonKey, backButton);
