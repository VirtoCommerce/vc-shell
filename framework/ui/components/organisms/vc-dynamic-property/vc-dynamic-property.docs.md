...
| `hideLabel`          | `boolean`                    | `false`    | Hides the control's own visible label so the consumer can render its own label row (e.g. to place an action next to the property name). Validation and the placeholder fallback are unaffected. The display name is forwarded as the control's accessible name for eight control families: short text, number, integer, long text, boolean, datetime, colour, and the fallback used for an unrecognized `valueType`. Dictionary and multivalue properties (VcSelect, VcMultivalue) and `Measure` properties (VcInputDropdown) are **not** covered — their focusable triggers carry no accessible name, so those properties still need a visible label. |
