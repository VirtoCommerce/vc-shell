...
| `hideLabel`          | `boolean`                    | `false`    | Hides the control's own visible label so the consumer can render its own label row (e.g. to place an action next to the property name). The display name is still forwarded as the control's accessible name; validation and the placeholder fallback are unaffected. |
